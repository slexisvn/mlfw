...
- **The schedule as a *partial* function** (Definition 38.1): each primitive refuses rather than repairs, so a search composing sound primitives cannot produce a wrong program however badly it searches. **That guarantee is conditional on all twenty-two being sound, and one is not** — `rfactor` re-brackets a reduction, which is its purpose and which on floats is an N2 transformation. §38.3 states the obligation, and Part VIII inherits the exception.
