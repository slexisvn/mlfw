...
- **Nothing checks a binding against the device's dimension limits.** `maxBlockDimY`, `maxGridDimY` and `maxGridDimZ` are declared on every GPU target ([`target.ts:73`](../../../src/compiler/support/target.ts)) and read by nothing in `src/compiler/`. A schedule binding 70,000 iterations to `blockIdx.y` would be accepted, compiled, and fail at launch.
