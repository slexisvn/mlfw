...
| [34](ch34-lowering-rules/README.md) | Lowering rules | 96 operations, 68 rules. How is one selected, and what happened to the other 28? |
...
A graph says *what*; hardware runs *how*. Somewhere a step has to invent an iteration order and a set of addresses, and that step is a function rather than a bijection — which is why every decision needing dataflow has already been made by the time it runs (Chapter 32).

What comes out is written in a language of 21 node kinds. The loops carry the plan; a `block` carries the computation, each of its axes declared spatial or reducing. That declaration is what lets a scheduler decide legality without running an analysis, and nothing verifies it (Chapter 33).

The translation itself is 68 operations' worth of coverage, selected by a two-level priority scheme, most of it one shared eleven-line skeleton plus a callback. The 28 operations with no rule are exactly the 21 the decomposition pass removes and the 7 the driver handles — an arithmetic that closes, and whose three classes Definition 34.6 pins down so later parts quote the same numbers (Chapter 34).

Every rule builds a subscript, and subscripts are affine forms. Flattening a coordinate is writing a mixed-radix numeral, unflattening is reading one, and a division by `c` vanishes exactly when `c` falls on a digit boundary. That is why one reshape compiles to a bare index and another pays a division per element — measured here at 1.10× on a pure copy (Chapter 35).

Given two such subscripts, deciding whether they can name the same element is a Diophantine equation, answered by three tests of decreasing precision. The answer is a direction vector saying which loops may be reordered; the reduction axis of every accumulation is the one that may not (Chapter 36).

All of it rests on interval arithmetic over the loop extents: sound in one direction, unable to relate two variables, and load-bearing not only for deleting `pad`'s two redundant comparisons but for substituting truncating division for floor division — legal exactly where the dividend is provably non-negative (Chapter 37).
