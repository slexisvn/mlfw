...
The consuming half is a second `build` path ([`backward_builder.ts:422`](../../../src/compiler/ad/backward_builder.ts)), and its loop is the theorem ([`backward_builder.ts:532`](../../../src/compiler/ad/backward_builder.ts)):
...
- **`CheckpointPolicy` is unreachable from `compileWithBackward`.** [`compile_backward.ts:103`](../../../src/tracing/compile_backward.ts) constructs a `RematPolicy` and never looks for a checkpoint policy; `_compileSeparate` and `_compileJoint` pass only `rematPolicy` to the builders. The four segmenters, `computeBoundaries`, and both `_buildCheckpointed` paths are live code with tests and no production caller. This is the same shape as Chapter 21's `nativeOps`, and the shape Chapter 25's `layoutAwareOps` had until a target declared it: designed, correct, unwired.
- **`scanCheckpoint` is unreachable the same way.** `BackwardBuilderOpts` has a third field for checkpointing *inside* a `scan` ([`backward_builder.ts:47`](../../../src/compiler/ad/backward_builder.ts)), consumed by Chapter 31's scan differentiator, and no caller sets it either.
