...
- **One regime.** A single β across all sizes, which the fit in §4.3 shows is false: `ns/element` is still falling at a million elements, and the affine model carries a ~30% worst-case residual across the measured range. Read α and β as ballpark constants, not as calibrated parameters.
...
- **Peaks are the hardware's, not the runtime's.** The 7.76 GB/s measured in §4.5 is what this eager JavaScript path achieves, not what the machine can do.
