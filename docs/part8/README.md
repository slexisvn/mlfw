...
Part VII gave the compiler a vocabulary for saying how a loop nest should run, and a guarantee with one exception. The guarantee is Proposition 38.4: any sequence of the twenty-two primitives that does not throw produces a program computing what the original computed. The exception is `rfactor`, which **reassociates a reduction** — not a defect but the whole point of the primitive, and on floats an N2 transformation offered at N1 without asking.

**This part inherits that exception**, in the worst possible setting: a search runs thousands of schedules nobody reads, keeps whichever measured fastest, and writes it to a database for reuse. Splitting a reduction across accumulators is *faster*, so a search that offers `rfactor` will tend to select it. Where a chapter below says a search "cannot produce a wrong kernel", read it as *cannot produce a wrong kernel by composing sound primitives* — which is the theorem — and check it against that exception, which is the practice. §48.4 is where a cached record replays one.

What Part VII did not give is a way to choose. Nine scheduling rules pick one arrangement per block from the target's declared attributes, and Chapter 40 measured what that costs on a matmul whose extents happen not to suit the rule.

This part replaces the choice with a search. That is a different kind of compiler activity from everything before it. A pass either fires or does not, and its correctness argument is about the program; a search has a *budget*, a *sample*, an *estimate* and a *result that could have been better*. Nothing here is provably optimal, and for most of the space nothing needs to be: where a sketch is a composition of *sound* Part VII primitives, soundness composes and the search can only be slow, never wrong. That is what turns an intractable correctness problem into a tractable engineering one — conditional, as above, on the primitives being sound. Chapter 45 names the two sketches to which the composition argument does not apply, one because it reassociates floating point and one because it does not call primitives at all.
...
One of the twenty-one is not a seam at all but should be read alongside them, because it is what makes the seams matter rather than merely untidy: the search can select an `rfactor` that changes the answer (above).

Three shapes recur among the seam findings. A component is written against a block shape the lowering rules do not emit, so it is derived, counted, sampled and always refused — the SSRSRS tiling sketch and the producer–consumer fusion sketch are both this, and between them they account for most of a matmul's advertised space. An objective is constant over the space it is meant to rank: the analytic cost model returns one value for all 2,304 points of the multi-level tiling space, provably and by measurement. And a generator's low bits collapse a choice: the LCG's period-4 residue makes the initial population a single sketch, which at the shipped default seed is the sketch that always throws.

None of *the twenty seam findings* produces a wrong kernel, because a searched schedule that fails is replaced by a rule-produced one and every end-to-end test asserts the answer rather than the schedule.

**That sentence is about the seam findings and it must not be read as a claim about the part as a whole**, because it sits one paragraph away from the two inherited exceptions above — `rfactor`, which reassociates, and the register-blocked GPU sketch, which calls no primitive at all — and the two kinds are easy to run together. A schedule that *throws* is replaced. A schedule that *succeeds and changes the answer* is not — nothing between the tuner and the emitted kernel compares its numerics against the untuned program. `rfactor` on a float reduction is exactly that case, and the whole path is documented rather than hypothetical: `createRfactorSketch` calls it (Chapter 45, Counterexample 45.8), it succeeds and is validator-clean, it is faster because splitting a reduction across four accumulators is faster, and §48.4's cache stores it as a `(sketchName, params)` pair and replays it on the next compilation. Chapter 41 §41.5 measures what it does to the numbers: eight `f32` values whose serial sum is `3` and whose four-way rfactored sum is `6`. Nothing between the sketch and the emitted kernel compares the two.

Note the boundary that `rfactor`'s own checks draw. Deriving the identity from the operator and validating the accumulation pattern (Chapter 41 §41.6) close the ways it could produce a *nonsensical* kernel. The way it produces a *differently-rounded* one stays open, and cannot close without changing what the primitive means.

So the honest summary of Part VIII's correctness position is: **the search cannot produce a schedule that fails; it can produce one that computes a different number, at N2, without saying so.** Each seam finding is named with its file and line in the chapter that meets it, each is reproducible by a lab here, and all twenty-one — the twenty seams plus `rfactor` — are collected in the outline's [Appendix E](../OUTLINE.md).
