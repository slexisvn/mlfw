...
One of the twenty is not a seam at all but should be read alongside them, because it is what makes the seams matter rather than merely untidy: the search can select an `rfactor` that changes the answer (above).
...
So the honest summary of Part VIII's correctness position is: **the search cannot produce a schedule that fails; it can produce one that computes a different number, at N2, without saying so.** Each seam finding is named with its file and line in the chapter that meets it, each is reproducible by a lab here, and all twenty are collected in the outline's [Appendix E](../OUTLINE.md).
