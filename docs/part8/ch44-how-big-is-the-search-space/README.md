...
**Note the form of the conclusion, because the tempting stronger version is false.** It is easy to write "if their optima differ, `h`'s regret on at least one of them is the full gap between the two optima", and that does not follow. Regret is measured against each workload's *own* optimum, not against the distance between the two optima, and the two are unrelated quantities. Concretely: suppose `A`'s optimum is point 1 at 10 ms and `B`'s is point 2 at 10 ms, and that point 3 runs at 10.1 ms on *both*. The two optima are different points; a heuristic returning point 3 has regret 0.1 ms on each. Nothing forces a blind heuristic to pay a large price merely because the optima disagree — what it pays depends on how *flat* the space is near them, which is a property of the workloads and not of the heuristic.

So the proposition says what it says and no more: **a heuristic is constant where it is blind, so it cannot be simultaneously optimal for two workloads it cannot distinguish unless they share an optimum.** That is enough to motivate search, and it is worth keeping the weaker form because the flat case is common — §44.6 measures eight points of a real matmul space spanning only 1.08×, which is exactly a region where a blind choice costs almost nothing. The argument for measuring is that you do not know in advance which regime you are in.
...
`staticExtent` returns `null` for a non-constant extent, and `createMultiLevelTilingSketch` refuses the whole sketch if any axis has one ([`tiling.ts:106`](../../../src/compiler/autotune/tiling.ts)). That is the point at which Chapter 62's symbolic shapes leave the search space entirely.

Say why carefully, though: a runtime integer has divisors like any other, and `n = 1024` has exactly as many whether the compiler knows the value or not. What is missing is not divisors but **knowledge of them at the moment the sketch is derived**. `createMultiLevelTilingSketch` enumerates ordered factorisations of a *literal*, so with a symbolic extent it has nothing to factor and refuses. The available alternatives are the ones every dynamic-shape compiler reaches for — tune against a representative value and guard on it, tune per size bucket, or emit a tile size the runtime supplies — and none is implemented here. "A dynamic extent cannot be factored at compile time" is the accurate statement; "a dynamic extent has no divisors" confuses the compiler's ignorance with a property of the number.
...
- **The reduction axis is never tiled by the default CPU structure.** `CPU_TILING` has one `R` level ([`tile_structure.ts:13`](../../../src/compiler/autotune/tile_structure.ts)), and `F(n,1) = 1`. The structure that does have two, `CPU_TILING_SSRSRS`, cannot be applied to any lowered block (Chapter 45). So the standard cache-blocking of a matrix multiply — block `M`, block `N`, *block `K`* — is not in this compiler's reachable search space at all.
