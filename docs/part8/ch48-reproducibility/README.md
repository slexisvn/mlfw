...
- **Nothing records which compiler produced a record.** `CODEGEN_VERSION` covers the code generator, and Chapter 47 showed its `undefined` hole; there is no equivalent for the schedule primitives, so a change to what `split` does would silently change what every stored trace means. For the parameter-based path the corresponding risk is a change to what a sketch of a given name does, which is equally unversioned.
