...
- **Two call paths route around the manager entirely.** `LayoutAnalysis.compute(graphFunc, { useDef }, this._policy)` is called directly by the layout pass ([`layout_transform.ts:59`](../../../src/compiler/passes/layout/layout_transform.ts)), and the AD builders call `UseDefAnalysis.compute` directly ([`backward_builder.ts:209`](../../../src/compiler/ad/backward_builder.ts)). Neither result is ever cached. The layout case is forced: the analysis takes a third *policy* argument, and `AnalysisCtor.compute` has room for exactly two — the protocol has no way to express a parameterized analysis, so the parameterized one opted out.
