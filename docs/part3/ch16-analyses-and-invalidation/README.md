...
It is worth checking the claim rather than accepting it, because Definition 16.2 is a proof obligation. `MemoryEffectResult` is a map keyed by `Operation` and by `Value`, recording each operation's side-effect kind ([`memory_effect.ts:95`](../../../src/compiler/analysis/memory_effect.ts)). DCE only *erases* operations; it never adds one, never rewires an operand, and never changes an operation's opcode or attributes. So for every operation that survives DCE, its entry is unchanged, and the entries left behind for erased operations are unreachable — nothing can ask about an operation that is no longer in the function. The claim holds.
