import {
  tensor, Module, ReLU, compile, trace, printModule, CPUTarget, TraceLevel, manual_seed,
} from '../../../../dist/index.node.js';

manual_seed(0);

const x = tensor([[1, 2], [3, 4]]);
const relu = new ReLU();

class Wasteful extends Module {
  forward(a) {
    const scaled = relu.forward(a.mul(2).add(1));
    const again = relu.forward(a.mul(2).add(1));
    const unused = a.exp().log();
    return scaled.add(again);
  }
}

console.log('=== traced ===');
console.log(printModule(await trace((a) => new Wasteful().forward(a), [x])));

const rows = [];
let pending = null;
let ir = null;

const compiled = compile(new Wasteful(), [x], {
  target: CPUTarget(),
  fusion: { enabled: false },
  trace: {
    level: TraceLevel.DEBUG,
    irSnapshot: { afterGraphPasses: true },
    sink: (e) => {
      if (e.type === 'pass_detail') pending = e;
      if (e.type === 'pass') { rows.push({ ...e, detail: pending }); pending = null; }
      if (e.type === 'ir_snapshot') ir = e.text;
    },
  },
});
await compiled._ready;

console.log('=== what each pass did ===');
let round = 0;
for (const e of rows) {
  if (e.passName === 'canonicalize') round++;
  if (!e.changed) continue;
  const d = e.detail;
  const detail = !d ? '' : Object.entries(d)
    .filter(([k]) => !['type', 'passName', 'level', 'timestamp'].includes(k))
    .map(([k, v]) => `${k}=${v}`).join(' ');
  console.log(`  round ${round}  ${e.passName.padEnd(20)} ${e.opCountBefore} -> ${e.opCountAfter} ops   ${detail}`);
}

console.log('\n=== after graph passes ===');
console.log(ir);
