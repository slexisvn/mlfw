...
- **Anything without a preference resets to row-major** ([`layout_analysis.ts:93`](../../../src/compiler/analysis/layout_analysis.ts)). A `reshape` between two convolutions therefore breaks an NHWC chain and forces two conversions where a smarter propagation would carry the layout through. Only `ELEMENTWISE` operations inherit.
