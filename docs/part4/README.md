...
| [22](ch22-fusion-why/README.md) | Fusion I: why it is the single most valuable optimization | Where does the 2.5× come from, and why does it not depend on tensor size? |
...
- **Decomposition to primitives** (Chapter 21), which is why Part VI needs 66 lowering rules rather than 96 — 21 of the remaining 30 operations are rewritten away before lowering sees them, and 9 are handled structurally — and why Part V needs gradient rules only for primitives.
...
Unlike Parts II and III, half of these measure time. The IR they print is deterministic; the milliseconds are not, and each chapter says which of its numbers should reproduce and which are machine-specific. Where a lab reports a speedup, what should hold anywhere is the *ordering* and the rough magnitude — 2.5× for fusing a four-operation chain, not 2.55×.

Several labs reach past the public surface in the way Part III established: `compile` accepts a `passContext`, and a target object can be handed extra attributes before being passed in. Chapters 21, 24 and 25 use the second of those to enable machinery no shipped target enables, and say so where they do.

## A note on what this part found

Five of the eight chapters end with a measurement that contradicts something the code implies, and they yield six findings between them. `x + 0` and `x * 0` changed results on non-finite inputs without any flag being set; a dead `scatter` and its whole index subgraph survive a pass whose job is removing them; the default fusion strategy left 2.4× on a three-operation program; layout transformation is inert unless you set a field no target sets, and slower when you do; a memory budget reported success without meeting it; quantization's only escape from a lossy default range could not be reached through the public API at all.

Four of the six are now fixed, and each chapter shows the measurement on both sides of its fix rather than only after it. That is the point of keeping the finding in the text: the interesting content of §20.6 is not that `AddZero` needs a dtype check, it is the four-layer chase that located it, and the interesting content of §24.5 is that a one-character `||` cost 2.4× while failing no test and printing nothing. Two findings — the dead `scatter` (§19.7) and the inert layout pass (§25.6) — are still open, and are described in the present tense for that reason. Each finding names the file and line, each is reproducible by a lab in this part, and each is carried in the outline's Appendix E, where the closed ones have moved to a separate list.
