...
Five of the eight chapters end with a measurement that contradicts something the code implies, and they yield six findings between them. `x + 0` and `x * 0` changed results on non-finite inputs without any flag being set; a dead `scatter` and its whole index subgraph survive a pass whose job is removing them; the default fusion strategy left more than a factor of two on a three-operation program; layout transformation is inert unless you set a field no target sets, and slower when you do; a memory budget reported success without meeting it; quantization's only escape from a lossy default range could not be reached through the public API at all.
