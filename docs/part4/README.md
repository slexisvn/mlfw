...
| [21](ch21-decomposition/README.md) | Decomposition | Why would a compiler deliberately make the graph eight times bigger? |
...
- **Decomposition to primitives** (Chapter 21), which is why Part VI needs only 32 lowering rules for 96 operations, and why Part V needs gradient rules only for primitives.
...
Six of the eight chapters end with a measurement that contradicts something the code implies. `x + 0` and `x * 0` change results on non-finite inputs without any flag being set; a dead `scatter` and its whole index subgraph survive a pass whose job is removing them; the default fusion strategy leaves 2.4× on a three-operation program; layout transformation is inert unless you set a field no target sets, and slower when you do; a memory budget reports success without meeting it; quantization without calibration is 18% wrong and says nothing.

None of these is fixed in the code this book describes, and that is deliberate: a book that quietly patches its subject between chapters is one you cannot check against the repository. Each finding names the file and line, each is reproducible by a lab in this part, and each is carried into the outline's Appendix E so it does not get lost.
