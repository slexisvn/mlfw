...
There are three fusion engines in this compiler and two auxiliary passes. They see the same graph, use the same legality checks and the same cost model, and on a three-operation program they produce three different answers — one of them 2.4× faster than the default.
...
Fusion is **not** monotone in the cost sense: merging `A` with `B` changes the group's inputs and outputs, so an edge that was unprofitable can become profitable, and vice versa. It *is* monotone in the cycle sense — contracting more can only create cycles, never remove them. The priority engine therefore keeps a heap it re-pushes into, and validates popped entries against a version counter rather than trusting them:

> **Definition 24.4 (Stale candidate, stated here).** A heap entry naming two groups is *stale* if either group has been merged since the entry was pushed. A stale entry must be discarded rather than applied, because its recorded benefit was computed for groups that no longer exist.
...
[`buildGraphPipeline`](../../../src/compiler/pipeline/graph_pipeline.ts) selects between them on a config string ([`graph_pipeline.ts:67`](../../../src/compiler/pipeline/graph_pipeline.ts)):
...
| [`MultiOutputFusionPass`](../../../src/compiler/passes/fusion/multi_output_fusion.ts) | always | siblings sharing inputs | graph order |
| [`EpilogueFusionPass`](../../../src/compiler/passes/fusion/epilogue_fusion.ts) | target opt-in | `dot`/`conv` + elementwise tail | graph order |

### The priority engine

Three data structures ([`priority_fusion.ts:89`](../../../src/compiler/passes/fusion/priority_fusion.ts)):
...
`cycles` is Chapter 23. `version` implements Definition 24.4 — one counter per group representative, bumped on every merge. `groupOf` maps a representative to its group.
...
The main loop is Definition 24.4 in practice ([`priority_fusion.ts:191`](../../../src/compiler/passes/fusion/priority_fusion.ts)):
...
Three operations, all elementwise, all the same shape, on 256K-element tensors. Every pair is legal by Chapter 23. And:
...
Two of the three engines fuse **nothing**. The reason is in the explanation: `s` is used twice inside the candidate group, so the cost model charges it as shared memory ([`fusion_cost.ts:203`](../../../src/compiler/passes/fusion/fusion_cost.ts)) at its full tensor size — one megabyte — against a limit of 48 KiB.

The priority engine reaches a different answer for a structural reason. It merges *edges*, so it first forms `{add, mul}` — at which point `s` has one internal user and one external one, no shared-memory charge, and the merge passes. Then it tries to add the final `add`, which would make `s` internally reused, and *that* merge is refused. It stops one step short of the group the other two engines evaluated as a whole. The result is a two-output region: the kernel yields both `s` and `s*x`, and the final add reads them back.

Now the fourth run, which is the point of the lab:
...
Same engine, same graph, one number changed — and all three operations fuse into one kernel, **2.4× faster than the default configuration**.

So the 48 KiB limit cost this program 2.4×, and it is worth asking where the number came from. 48 KiB is a GPU shared-memory budget. This compilation targets a CPU, which has no shared memory at all. The plumbing explains it ([`priority_fusion.ts:62`](../../../src/compiler/passes/fusion/priority_fusion.ts)):

```ts
      maxSharedMemory: target.sharedMemoryBytes,
```

and the CPU target sets `sharedMemoryBytes` to `0` ([`target.ts:114`](../../../src/backend/target.ts)), and the cost model reads it as `config.maxSharedMemory || 49152` ([`fusion_cost.ts:62`](../../../src/compiler/passes/fusion/fusion_cost.ts)). **Zero is falsy.** A target saying "I have no shared memory" is indistinguishable from a target saying nothing, and the fallback is the GPU default.

This is a one-character class of bug — `||` where `??` was meant — and it is worth dwelling on because of where it lands. It does not produce a wrong answer, it does not fail a test, and it does not appear in any trace unless you are running the non-default strategy that explains its refusals. It costs 2.4× and reports nothing.

**Try this.** Set `sharedMemoryBytes` on the target to `1` instead of raising the cost model's limit, and confirm the fusion is refused for a limit of 1 rather than 49152 — which is the evidence that the `||` is what is being hit.
...
Twenty-one operations become three. Each `Linear` + `ReLU` is now a single operation carrying its epilogue as an attribute, tagged `["bias", "relu"]` so a backend can recognize the shape without re-deriving it.
...
- **`0 ||` makes "no shared memory" mean "48 KiB of shared memory".** §24.5 measures 2.4× on one three-operation program. Every target with `sharedMemoryBytes: 0` — CPU and WASM — inherits a GPU budget for a resource it does not have.
- **The launch weight is 1000 and is not target-derived.** [`fusion_cost.ts:38`](../../../src/compiler/passes/fusion/fusion_cost.ts) fixes `{ memory: 1, launch: 1000 }` for every target, so a saved launch is worth 5,000 bytes of traffic everywhere. A target may override it through the `fusionBenefitWeights` attribute ([`priority_fusion.ts:57`](../../../src/compiler/passes/fusion/priority_fusion.ts)); none does.
- **The strategy string has no validation.** Anything that is not `'dominator'` or `'priority'` silently selects `FusionPass` ([`graph_pipeline.ts:75`](../../../src/compiler/pipeline/graph_pipeline.ts)). A typo in a config becomes a different fusion engine rather than an error.
...
- **Priority fusion explains only its successes.** Chapter 18's finding, and §24.5 is why it matters: on the default strategy the diamond's refused merge produces no event at all. The refusals that cost you performance are visible only if you switch to a strategy you were not going to use.
...
- [`tests/compiler/passes/fusion/priority.test.js`](../../../tests/compiler/passes/fusion/priority.test.js) — heap ordering, the version-based staleness of Definition 24.4, and the weighted union.
