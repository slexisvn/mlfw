...
> **Every level answers a different question.** The Graph IR knows *what mathematics the program performs* and nothing about loops. TIR knows *what loops compute the result*, and indexes memory one dimension at a time — `A[i, j]`, never `A[i*8 + j]`. LIR knows *exactly which flat address is touched* and nothing about the mathematics. Compilers are structured this way because an optimization is easy to express at exactly one of these levels and awkward at every other.

Be careful with the middle one, because the tempting summary — "TIR knows nothing about layout" — is false, and later parts of the book depend on it being false. TIR carries `Buffer` objects with a shape, a dtype and a **memory scope** (`global`, `shared`, `local`); it carries the index expression at every access; and `storage_align`, one of the scheduling primitives in Chapter 41, exists precisely to change a buffer's padding. Those are all layout decisions and they are all made in TIR. What TIR withholds is narrower and more specific: **the flattening function**. It never commits to how the index tuple `(i, j)` becomes a single integer offset. Choosing that map — row-major, column-major, tiled, padded — is what Chapter 25 calls layout selection at the graph level and what LIR finally freezes. "TIR is not yet flat" is the accurate claim; "TIR knows nothing about layout" is the one to avoid.
...
Eagerly does not mean interpreted, and it does not mean "hand-written kernels" either — that is the summary to avoid. Most arithmetic operations reach the *same code generator the compiler uses*, one operation at a time, compiled on first use at each new shape and cached in [`src/dispatcher/jit_cache.ts`](../../../src/dispatcher/jit_cache.ts). Two families opt out and keep a hand-written implementation: **view operations** (`reshape`, `transpose`, `slice` …), which only rewrite shape and stride metadata, and **composite operations** (`sort`, `topk`, `cumsum` …), which are awkward to express as one graph operation. §4.2 walks that path and §4.4 measures the first call against the steady state. So what `compile()` adds is not compilation — that is already happening. It is *seeing more than one operation at a time*.

### Who owns the trace

One more piece of state is worth knowing about before Part I, because it explains a class of confusing behaviour. Tracing is driven by a single module-level variable, `_activeTracer` in [`tracer.ts:14`](../../../src/tracing/tracer.ts): an operation on a symbolic tensor consults it to find out where to append. Two things follow.

**There is one tracer, not a stack.** `activate()` assigns to that variable and `deactivate()` sets it back to `null` rather than restoring a previous value ([`tracer.ts:216`](../../../src/tracing/tracer.ts)). One trace at a time is the contract; starting a second one while the first is live is not supported, and the failure is silent rather than loud.

**Nested *regions* are a different mechanism, and they nest properly.** When a traced program reaches a `scan`, an `if` or a `while`, the body is traced into that operation's region by swapping the tracer's *builder* — the cursor that says where new operations are appended — and restoring it in a `finally` ([`tracer.ts:132`](../../../src/tracing/tracer.ts)). The tracer stays the same object throughout; only the insertion point moves. So "which phase owns the trace state" has two answers at two scales: the tracer is global and unstacked, while the insertion point is stacked and correctly restored even if the body throws. Chapter 31 depends on the second fact when it differentiates control flow.
...
**Op count is a crude measure of work.** Both `canonicalize` and `constant_fold` report `10 ops -> 10 ops`, and both changed the program substantially: one rewired a matrix multiply so that a transpose became unnecessary, the other replaced a broadcast with a constant of the final shape. Neither deleted anything, because deleting is not their job. Whenever you read a pass log, remember that the counter measures size, not effect.

**But size and change are two different columns, and only one of them is a contract.** The op count is a diagnostic; it can stay flat while a pass rewires the program completely, exactly as it did here. The pass *result* — `CHANGED` or `UNCHANGED` — is something else entirely: it is a declaration the pass makes about whether it touched the IR at all, and the pass manager acts on it. `CHANGED` invalidates the analyses the pass did not promise to preserve, and triggers re-verification; `UNCHANGED` skips both. So `canonicalize` and `constant_fold` here both report `CHANGED` despite the flat op count, and they must, because they edited the IR. A pass that does decisive work and reports `UNCHANGED` is not being modest — it is lying to the pass manager, and Chapter 14 shows what that costs: a stale analysis survives, and the verifier that eventually notices the damage blames whichever pass ran next. Read the count as a hint; read the result as a promise.
