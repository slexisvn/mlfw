import { CHART_METHOD_DOCS } from './chart/docs.js';
...
export function initNotebookDocs({ createCell } = {}) {
  createCellFn = createCell || null;
  inputEl = document.getElementById('docs-search');
...
export function updateNotebookDocs(languageData) {
...
export function setNotebookDocsError(message) {
  if (!listEl) return;
  listEl.innerHTML = '';
  listEl.append(emptyState(message || 'Tera docs unavailable.'));
}

function buildDocsSections(data) {
  const sections = [];
...
function addDocsSection(sections, title, items) {
...
function sortSections(sections) {
  ...
  return sections.sort((a, b) => {
    const ai = order.has(a.title) ? order.get(a.title) : SECTION_ORDER.length;
    const bi = order.has(b.title) ? order.get(b.title) : SECTION_ORDER.length;
    return ai - bi || a.title.localeCompare(b.title);
  });
}

function sectionForBuiltin(builtin) {
...
function sectionForType(typeName) {
...
function tagForBuiltin(builtin) {
  const name = builtin.name;
...
function isHiddenDocBuiltin(builtin) {
...
function hasCallableParams(signature) {
...
function renderSections() {
  for (const section of docsSections) {
    const visibleLimit = expanded.has(section.title) ? expanded.get(section.title) : INITIAL_ITEMS;
...
function renderSearch(query) {
  ...
  for (const match of matches) {
    if (!grouped.has(match.section)) grouped.set(match.section, []);
    grouped.get(match.section).push(match.item);
  }
...
function sectionNode(section, items, opts = {}) {
  ...
  if (opts.remaining && !opts.search) {
    const current = expanded.has(section.title) ? expanded.get(section.title) : INITIAL_ITEMS;
...
function haystack(item, sectionTitle) {
...
function tensorMemberExample(item) {
...
function dataframeMemberExample(item) {
...
function columnMemberExample(item) {
...
function listMemberExample(item) {
  const name = item.name;
  const mutating = {
    append: 'items = [1, 2]\nitems.append(3)\nitems',
    extend: 'items = [1]\nitems.extend([2, 3])\nitems',
...
function stringMemberExample(item) {
  const name = item.name;
  const map = {
    upper: '"hello".upper()',
    lower: '"HELLO".lower()',
...
function dictMemberExample(item) {
  const name = item.name;
  const map = {
...
function sampleReceiver(typeName) {
  const map = {
    Tensor: 'x',
    Model: 'model',
    DataFrame: 'df',
