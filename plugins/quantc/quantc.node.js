...
var gbm = {
  name: "gbm",
  terminal(context2) {
    const g = context2.graph;
    const half = g.constant(0.5);
    const variance = g.mul(g.mul(half, context2.vol), context2.vol);
    const drift = g.mul(g.sub(context2.rate, variance), context2.maturity);
    const diffusion = g.mul(g.mul(context2.vol, g.sqrt(context2.maturity)), context2.normals);
    const logTerminal = g.add(g.add(g.log(context2.spot), drift), diffusion);
...
function buildEuropeanGraph(payoff2, model) {
  ...
  const context2 = { graph, spot: spot2, rate, vol, maturity, normals };
  const terminal2 = models.get(model).terminal(context2);
...
// src/runtime/cuda/driver.ts
import { createRequire } from "node:module";
import { readdirSync } from "node:fs";
import { join } from "node:path";
var require4 = createRequire(import.meta.url);
var koffi = require4("koffi");
function locateNvrtc() {
  const root = process.env.CUDA_PATH;
  if (root === void 0) throw new Error("CUDA_PATH is not set");
  const binary2 = join(root, "bin");
  const match = readdirSync(binary2).find((name) => /^nvrtc64_\d+_\d+\.dll$/.test(name));
  if (match === void 0) throw new Error("nvrtc runtime compiler not found");
  return join(binary2, match);
}
var COMPUTE_CAPABILITY_MAJOR = 75;
var COMPUTE_CAPABILITY_MINOR = 76;
var driver = null;
var nvrtc = null;
var context = null;
var architecture = "compute_70";
function loadDriver() {
...
function compileToPtx(nvrtcLib, source, architecture2) {
  const program = [null];
  check(nvrtcLib.nvrtcCreateProgram(program, source, "kernel.cu", 0, null, null), "nvrtcCreateProgram");
  const options = [`--gpu-architecture=${architecture2}`];
  ...
  if (status !== 0) {
    ...
    const log2 = Buffer.alloc(Number(logSize[0]));
    nvrtcLib.nvrtcGetProgramLog(program[0], log2);
    throw new Error(`nvrtcCompileProgram failed: ${log2.toString("utf8")}`);
  }
  const ptxSize = [0];
...
function launchReductionKernel(launch, scalarValues, batchData, batchSize, targetArchitecture) {
  ...
  const devicePointers = [];
  const argumentBuffers = [];
  ...
  for (const id of launch.batchInputs) {
    ...
    if (data === void 0) throw new Error(`missing batch input ${id}`);
    const pointer = [0];
...
function timeOf(expr, context2) {
  if (expr.kind === "ident" && expr.name === context2.eventVar) return context2.eventTime;
  const t = evalConst(expr, context2.paramDefaults);
  return t <= 0 ? t : t - context2.timeShift;
}
...
function lowerExpr(expr, context2) {
  const g = context2.graph;
  switch (expr.kind) {
    case "num":
      return g.constant(expr.value);
    case "ident": {
      if (expr.name === context2.eventVar) return context2.timeValue;
      const value = context2.env.get(expr.name);
      if (value === void 0) throw new Error(`unknown identifier '${expr.name}'`);
      return value;
    }
    case "unary": {
      const operand = lowerExpr(expr.operand, context2);
      return expr.op === "not" ? g.sub(g.constant(1), operand) : g.neg(operand);
    }
    case "binary": {
      if (expr.op === "and") return g.mul(lowerExpr(expr.left, context2), lowerExpr(expr.right, context2));
      if (expr.op === "or") {
        const a = lowerExpr(expr.left, context2);
        const b = lowerExpr(expr.right, context2);
        return g.sub(g.add(a, b), g.mul(a, b));
      }
      const op = BINARY_OPS[expr.op];
      if (op === void 0) throw new Error(`unknown operator ${expr.op}`);
      return g.emit(op, [lowerExpr(expr.left, context2), lowerExpr(expr.right, context2)], {}, op);
    }
    case "ternary": {
      const condition = lowerExpr(expr.cond, context2);
      return blend(g, condition, lowerExpr(expr.whenTrue, context2), lowerExpr(expr.whenFalse, context2));
    }
    case "call":
      return lowerCall(expr, context2);
    case "let": {
      const value = lowerExpr(expr.value, context2);
      const env = new Map(context2.env);
      env.set(expr.name, value);
      return lowerExpr(expr.body, { ...context2, env });
    }
  }
}
function assetFixings(name, context2) {
  const asset = context2.assets.get(name);
  if (asset === void 0) throw new Error(`unknown underlying '${name}'`);
  return asset.fixings;
}
function lowerCall(expr, context2) {
  const g = context2.graph;
  if (context2.assets.has(expr.callee)) return fixingAt(assetFixings(expr.callee, context2), timeOf(expr.args[0], context2));
  if (expr.callee === "bond") {
    const target = expr.args[0];
    const name = target.kind === "ident" ? target.name : context2.assetOrder[0];
    const t = context2.eventTime;
    const maturity = timeOf(expr.args[1], context2);
    const key = `${name}|${t}|${maturity}`;
    const cached = context2.bondCoeffs.get(key);
    if (cached !== void 0) return cached;
    const factors = context2.rateFactors.get(name)?.get(t);
    ...
    context2.rateScalarInputs.push({ input: constInput, kind: "bondConst", asset: name, time: t, maturity });
    let exponent = constInput;
    for (let k = 0; k < factors.length; k += 1) {
      const loading = g.input("scalar", `bond_load${k}_${name}_${t}_${maturity}`);
      context2.rateScalarInputs.push({ input: loading, kind: "bondLoading", asset: name, time: t, maturity, factorIndex: k });
      exponent = g.add(exponent, g.mul(loading, factors[k]));
    }
    const result = g.exp(exponent);
    context2.bondCoeffs.set(key, result);
    return result;
  }
  if (expr.callee === "runningMax" || expr.callee === "runningMin") {
    const target = expr.args[0];
    const name = target.kind === "ident" ? target.name : context2.assetOrder[0];
    const limit = expr.args.length > 1 ? timeOf(expr.args[1], context2) : context2.eventTime;
    const relevant = assetFixings(name, context2).filter((f) => f.time <= limit + 1e-9 && f.time > 0);
  ...
  if (expr.callee === "average") {
    const target = expr.args[0];
    const name = target.kind === "ident" ? target.name : context2.assetOrder[0];
    const relevant = assetFixings(name, context2).filter((f) => f.time <= context2.eventTime + 1e-9 && f.time > 0);
  ...
  const args = expr.args.map((arg) => lowerExpr(arg, context2));
...
function lowerStmt(stmt, context2) {
  const g = context2.graph;
  switch (stmt.kind) {
    case "assign": {
      const value = lowerExpr(stmt.expr, context2);
      const old = context2.env.get(stmt.name) ?? g.constant(0);
      context2.env.set(stmt.name, blend(g, context2.condition, value, old));
      return;
    }
    case "if": {
      const indicator = lowerExpr(stmt.cond, context2);
      const thenContext = { ...context2, condition: g.mul(context2.condition, indicator) };
      for (const inner of stmt.body) lowerStmt(inner, thenContext);
      if (stmt.otherwise !== null) {
        const elseContext = { ...context2, condition: g.mul(context2.condition, g.sub(g.constant(1), indicator)) };
        for (const inner of stmt.otherwise) lowerStmt(inner, elseContext);
      }
      return;
    }
    case "pay": {
      let amount = lowerExpr(stmt.amount, context2);
      if (stmt.currency !== null) amount = g.mul(amount, fixingAt(assetFixings(stmt.currency, context2), timeOf(stmt.date, context2)));
      const date = timeOf(stmt.date, context2);
      const discount = context2.stochasticDiscountFor?.(date) ?? context2.discountFor(date);
      const contribution = g.mul(g.mul(g.mul(context2.condition, context2.alive.value), discount), amount);
      context2.cashflow.value = g.add(context2.cashflow.value, contribution);
      return;
    }
    case "stop":
      context2.alive.value = g.mul(context2.alive.value, g.sub(g.constant(1), context2.condition));
      return;
    case "exercise": {
      const mask = g.input("batch", `exercise_${context2.eventTime}`);
      const payStmt = stmt.body.find((inner) => inner.kind === "pay");
      const intrinsic3 = payStmt !== void 0 && payStmt.kind === "pay" ? lowerExpr(payStmt.amount, context2) : g.constant(0);
      const level = fixingAt(assetFixings(context2.assetOrder[0], context2), context2.eventTime);
      context2.exercises.push({ date: context2.eventTime, intrinsic: intrinsic3, underlying: level, mask });
      const exerciseContext = { ...context2, condition: g.mul(context2.condition, mask) };
      for (const inner of stmt.body) lowerStmt(inner, exerciseContext);
      context2.alive.value = g.mul(context2.alive.value, g.sub(g.constant(1), exerciseContext.condition));
      return;
    }
  }
}
function compileProduct(product, options = {}) {
  ...
  for (const instance of instances) {
    ...
    for (const stmt of instance.body) lowerStmt(stmt, context2);
  }
...
function combineCosts(...models2) {
  return (trade, weights, context2) => models2.reduce((sum, model) => sum + model(trade, weights, context2), 0);
}
var COSTS = {
