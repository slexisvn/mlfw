...
var RelationBuilder = class {
  ...
  _initFrom(rows) {
    ...
    this._names = declaredSchema ? declaredSchema.map((c) => c.name) : rows.length > 0 ? Object.keys(rows[0]) : [];
    this._extractByIndex = Array.isArray(rows[0]);
    if (declaredSchema) {
      this.schema = declaredSchema.map((c) => ({ name: c.name, dataType: c.dataType }));
    } else {
      const extract = this._extractByIndex ? (row, i) => row[i] : (row, i) => row[this._names[i]];
  ...
  appendRows(rows) {
    ...
    const extract = this._extractByIndex ? (row, i) => row[i] : (row, i) => row[names[i]];
    for (const row of rows) {
      ...
      this._chunk.appendRow(schema.map((col2, i) => coerceForColumn(extract(row, i), col2.dataType)));
    }
    return this;
  }
  finish() {
    ...
    this._finished = true;
    if (this.schema === null) {
      this.schema = this._declaredSchema ? this._declaredSchema.map((c) => ({ name: c.name, dataType: c.dataType })) : [];
      this._chunk = DataChunk.fromSchema(this.schema, DEFAULT_CHUNK_SIZE);
    }
...
async function registerStreamingTable(engine, name, schemaOrBatches, maybeBatches) {
