...
export class Precision extends Metric {
  constructor({ task = 'multiclass', numClasses = 2, average = 'macro' } = {}) {
    super();
    this._task = task;
...
function updateConfusionCounters(preds, target, tp, fp, fn, support, task, numClasses) {
...
function computePrecisionPerClass(tp, fp, numClasses) {
...
function computeRecallPerClass(tp, fn, numClasses) {
  const result = new Float64Array(numClasses);
  for (let c = 0; c < numClasses; c++) {
...
function computePrecision(tp, fp, support, numClasses, average) {
  const perClass = computePrecisionPerClass(tp, fp, numClasses);
  return aggregate(perClass, support, numClasses, average);
}

function computeRecall(tp, fn, support, numClasses, average) {
  const perClass = computeRecallPerClass(tp, fn, numClasses);
  return aggregate(perClass, support, numClasses, average);
}

function aggregate(perClass, support, numClasses, average) {
