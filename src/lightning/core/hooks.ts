...
import type { HyperparameterRecord, LightningModuleLike, NumericMetricRecord } from '../types.js';
import type { MetricValue } from '../types.js';
import type { ReduceFnName, TrainerState } from './state.js';
...
type HookFn = (this: Callback, ...args: unknown[]) => void;
type LogEntry = {
  value: MetricValue;
  onStep: boolean;
  onEpoch: boolean;
...
export class CallbackConnector {
  ...
  dispatch(hookName: HookName, ...args: unknown[]): void {
    for (let i = 0; i < this._callbacks.length; i++) {
      const fn = (this._callbacks[i] as unknown as Record<string, unknown>)[hookName];
      if (typeof fn === 'function') (fn as HookFn).apply(this._callbacks[i], args);
    }
  }
...
export class LoggerConnector {
  ...
  constructor(loggers: Logger | Logger[] = [], state: TrainerState) {
  ...
  drain(model: LogModel): void {
    ...
    for (const [name, entry] of buf) {
      const { value, onStep, onEpoch, reduceFx, progBar } = entry as LogEntry;
      ...
      if (progBar) {
        ...
        this._state._progBarMetrics.set(name, v as number);
      }
    }
