...
export class IValue {
  ...
  static tensor(t) { return new IValue(IValueTag.TENSOR, t); }
  static int(n) { return new IValue(IValueTag.INT, n); }
  static float(n) { return new IValue(IValueTag.FLOAT, n); }
  static bool(b) { return new IValue(IValueTag.BOOL, b); }
  static intList(arr) { return new IValue(IValueTag.INT_LIST, arr); }
  static tensorList(arr) { return new IValue(IValueTag.TENSOR_LIST, arr); }
  static string(s) { return new IValue(IValueTag.STRING, s); }
  static none() { return new IValue(IValueTag.NONE, null); }
  static device(d) { return new IValue(IValueTag.DEVICE, d); }
  static dtype(d) { return new IValue(IValueTag.DTYPE, d); }
...
export class KernelFunction {
  ...
  static fromUnboxed(fn) {
    return new KernelFunction(null, fn);
  }

  static fromBoth(boxed, unboxed) {
  ...
  _callUnboxedAsBoxed(keySet, stack) {
    const args = stack.map(iv => iv.value);
    const result = this._unboxed(keySet, ...args);
    if (result === undefined || result === null) return [];
    return [_toIValue(result)];
  }
}

function _toIValue(val) {
  if (val instanceof IValue) return val;
  if (val && typeof val === 'object' && val._impl) return IValue.tensor(val);
  ...
  if (Array.isArray(val)) {
    if (val.length > 0 && val[0] && val[0]._impl) return IValue.tensorList(val);
