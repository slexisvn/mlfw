export enum DispatchKey {
  CPU = 0,
  GPU = 1,
  WASM = 2,
  META = 3,
  LAZY = 4,
  CUSTOM_0 = 5,
  CUSTOM_1 = 6,
  CUSTOM_2 = 7,
  CUSTOM_3 = 8,

  BATCHED = 20,
  VMAP = 24,
  FUNCTIONALIZE = 28,
  AUTOCAST = 32,

  AUTOGRAD = 40,
  AUTOGRAD_CPU = 41,
  AUTOGRAD_GPU = 42,
  AUTOGRAD_WASM = 43,

  TRACING = 48,

  NUM_KEYS = 49,
}

export type DispatchKeyValue = DispatchKey;

const _KEY_NAMES: Array<string | null> = new Array(DispatchKey.NUM_KEYS).fill(null);
for (const [name, val] of Object.entries(DispatchKey)) {
  if (name !== 'NUM_KEYS' && typeof val === 'number' && val < DispatchKey.NUM_KEYS) {
    _KEY_NAMES[val] = name;
  }
}

const _BACKEND_KEY_FOR_DEVICE: Readonly<Record<string, DispatchKeyValue>> = Object.freeze({
  cpu: DispatchKey.CPU,
  gpu: DispatchKey.GPU,
  wasm: DispatchKey.WASM,
  webgpu: DispatchKey.CUSTOM_0,
  meta: DispatchKey.META,
  lazy: DispatchKey.LAZY,
});

const _AUTOGRAD_KEY_FOR_BACKEND: Readonly<Partial<Record<DispatchKeyValue, DispatchKeyValue>>> = Object.freeze({
  [DispatchKey.CPU]: DispatchKey.AUTOGRAD_CPU,
  [DispatchKey.GPU]: DispatchKey.AUTOGRAD_GPU,
  [DispatchKey.WASM]: DispatchKey.AUTOGRAD_WASM,
});

export function backendKeyForDevice(deviceType: string): DispatchKeyValue {
  const k = _BACKEND_KEY_FOR_DEVICE[deviceType];
  if (k === undefined) throw new Error(`No backend key for device: ${deviceType}`);
  return k;
}

export function autogradKeyForBackend(backendKey: DispatchKeyValue): DispatchKeyValue {
  return _AUTOGRAD_KEY_FOR_BACKEND[backendKey] ?? DispatchKey.AUTOGRAD;
}

export class DispatchKeySet {
  private readonly _lo: number;
  private readonly _hi: number;

  constructor(lo: number, hi: number) {
    this._lo = lo | 0;
    this._hi = hi | 0;
  }

  static fromKey(key: DispatchKeyValue): DispatchKeySet {
    if (key < 32) return new DispatchKeySet(1 << key, 0);
    return new DispatchKeySet(0, 1 << (key - 32));
  }

  static fromKeys(...keys: DispatchKeyValue[]): DispatchKeySet {
    let lo = 0, hi = 0;
    for (let i = 0; i < keys.length; i++) {
      const k = keys[i];
      if (k < 32) lo |= (1 << k);
      else hi |= (1 << (k - 32));
    }
    return new DispatchKeySet(lo, hi);
  }

  add(key: DispatchKeyValue): DispatchKeySet {
    if (key < 32) return new DispatchKeySet(this._lo | (1 << key), this._hi);
    return new DispatchKeySet(this._lo, this._hi | (1 << (key - 32)));
  }

  remove(key: DispatchKeyValue): DispatchKeySet {
    if (key < 32) return new DispatchKeySet(this._lo & ~(1 << key), this._hi);
    return new DispatchKeySet(this._lo, this._hi & ~(1 << (key - 32)));
  }

  has(key: DispatchKeyValue): boolean {
    if (key < 32) return (this._lo & (1 << key)) !== 0;
    return (this._hi & (1 << (key - 32))) !== 0;
  }

  without(key: DispatchKeyValue): DispatchKeySet {
    return this.remove(key);
  }

  union(other: DispatchKeySet): DispatchKeySet {
    return new DispatchKeySet(this._lo | other._lo, this._hi | other._hi);
  }

  intersect(other: DispatchKeySet): DispatchKeySet {
    return new DispatchKeySet(this._lo & other._lo, this._hi & other._hi);
  }

  subtract(other: DispatchKeySet): DispatchKeySet {
    return new DispatchKeySet(this._lo & ~other._lo, this._hi & ~other._hi);
  }

  isEmpty(): boolean {
    return this._lo === 0 && this._hi === 0;
  }

  equals(other: DispatchKeySet): boolean {
    return this._lo === other._lo && this._hi === other._hi;
  }

  highestPriority(): number {
    if (this._hi !== 0) {
      return (63 - Math.clz32(this._hi));
    }
    if (this._lo !== 0) {
      return (31 - Math.clz32(this._lo));
    }
    return -1;
  }

  lowestPriority(): number {
    if (this._lo !== 0) {
      return 31 - Math.clz32(this._lo & -this._lo);
    }
    if (this._hi !== 0) {
      return 32 + 31 - Math.clz32(this._hi & -this._hi);
    }
    return -1;
  }

  count(): number {
    return _popcount32(this._lo) + _popcount32(this._hi);
  }

  *[Symbol.iterator](): Generator<DispatchKeyValue> {
    let hi = this._hi;
    while (hi !== 0) {
      const bit = 31 - Math.clz32(hi);
      yield bit + 32 as DispatchKeyValue;
      hi &= ~(1 << bit);
    }
    let lo = this._lo;
    while (lo !== 0) {
      const bit = 31 - Math.clz32(lo);
      yield bit as DispatchKeyValue;
      lo &= ~(1 << bit);
    }
  }

  toString(): string {
    const names = [];
    for (const k of this) {
      names.push(_KEY_NAMES[k] || String(k));
    }
    return `DispatchKeySet(${names.join(', ')})`;
  }
}

function _popcount32(n: number): number {
  n = n - ((n >> 1) & 0x55555555);
  n = (n & 0x33333333) + ((n >> 2) & 0x33333333);
  return (((n + (n >> 4)) & 0x0f0f0f0f) * 0x01010101) >> 24;
}

export const EMPTY_KEY_SET = new DispatchKeySet(0, 0);

export const BACKEND_KEY_SET = DispatchKeySet.fromKeys(
  DispatchKey.CPU, DispatchKey.GPU, DispatchKey.WASM,
  DispatchKey.META, DispatchKey.LAZY,
  DispatchKey.CUSTOM_0, DispatchKey.CUSTOM_1,
  DispatchKey.CUSTOM_2, DispatchKey.CUSTOM_3
);

export const AUTOGRAD_KEY_SET = DispatchKeySet.fromKeys(
  DispatchKey.AUTOGRAD, DispatchKey.AUTOGRAD_CPU,
  DispatchKey.AUTOGRAD_GPU, DispatchKey.AUTOGRAD_WASM
);

export const FUNCTIONALITY_KEY_SET = DispatchKeySet.fromKeys(
  DispatchKey.BATCHED, DispatchKey.VMAP,
  DispatchKey.FUNCTIONALIZE, DispatchKey.AUTOCAST,
  DispatchKey.AUTOGRAD, DispatchKey.AUTOGRAD_CPU,
  DispatchKey.AUTOGRAD_GPU, DispatchKey.AUTOGRAD_WASM,
  DispatchKey.TRACING
);
