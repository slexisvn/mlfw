...
export function inferReshape(sizes, strides, newSizes) {
  ...
  if (_canReshapeWithoutCopy(sizes, strides, resolved)) {
    return { sizes: resolved, strides: _computeReshapeStrides(sizes, strides, resolved), needsCopy: false };
  }

  return { sizes: resolved, strides: computeStrides(resolved), needsCopy: true };
}

function _canReshapeWithoutCopy(oldSizes, oldStrides, newSizes) {
  const oldNdim = oldSizes.length;
  const newNdim = newSizes.length;

  if (computeNumel(oldSizes) === 0) return true;

  let oi = 0, ni = 0;
  while (oi < oldNdim && ni < newNdim) {
    let oldChunk = oldSizes[oi];
    let newChunk = newSizes[ni];

    if (oldChunk === 1) { oi++; continue; }
    if (newChunk === 1) { ni++; continue; }
    ...
    while (oldChunk !== newChunk) {
      if (oldChunk < newChunk) {
        oi++;
        if (oi >= oldNdim) return false;
        if (oldStrides[oi] !== oldStrides[oi - 1] * oldSizes[oi - 1] &&
            oldSizes[oi - 1] !== 1) return false;
        oldChunk *= oldSizes[oi];
      } else {
        ni++;
        if (ni >= newNdim) return false;
        newChunk *= newSizes[ni];
      }
    }
    oi++;
    ni++;
  ...
  while (oi < oldNdim) {
    if (oldSizes[oi] !== 1) return false;
    oi++;
  }
  while (ni < newNdim) {
    if (newSizes[ni] !== 1) return false;
    ni++;
  }
...
function _computeReshapeStrides(oldSizes, oldStrides, newSizes) {
  const newStrides = computeStrides(newSizes);
  if (isContiguous(oldSizes, oldStrides)) return newStrides;

  const newNdim = newSizes.length;
  const result = new Array(newNdim);
  let oi = oldSizes.length - 1;
  let ni = newNdim - 1;

  while (ni >= 0) {
    while (ni >= 0 && newSizes[ni] === 1) {
      result[ni] = newStrides[ni];
      ni--;
    }
    if (ni < 0) break;

    while (oi >= 0 && oldSizes[oi] === 1) oi--;
    if (oi < 0) break;

    result[ni] = oldStrides[oi];
    let remaining = newSizes[ni];
    ni--;

    while (remaining < oldSizes[oi] && ni >= 0) {
      result[ni] = result[ni + 1] * newSizes[ni + 1];
      remaining *= newSizes[ni];
      ni--;
    }
    oi--;
  }
  ...
  while (ni >= 0) {
    result[ni] = ni < newNdim - 1 ? result[ni + 1] * newSizes[ni + 1] : 1;
    ni--;
  }
