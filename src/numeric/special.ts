...
type PpfOptions = { refineSteps?: number };
...
export function betaRegularized(a: number, b: number, x: number): number {
  if (x <= 0) return 0;
  if (x >= 1) return 1;
...
export function normalCdfScalar(z: number): number {
  return 0.5 * erfcScalar(-z / SQRT2);
}

export function normalPdfScalar(z: number): number {
  return Math.exp(-0.5 * z * z) / SQRT_2PI;
}

function ppfSeed(p: number): number {
  if (p < PPF_LOW) {
...
export function normalPpfScalar(p: number, opts: PpfOptions = {}): number {
