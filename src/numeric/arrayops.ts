...
import type { NumericArrayInput, NumericVectorInput } from './types.js';
...
function modeSlice(la: number, lb: number, mode: ConvMode): { offset: number; length: number } {
  const lFull = la + lb - 1;
...
function fftConvolve(a: Float64Array, b: Float64Array, mode: ConvMode, device: Device, dtype: DType): Tensor {
  const la = a.length;
  const lb = b.length;
  ...
  for (let k = 0; k < m; k++) {
    const ar = f64(A, 2 * k);
    const ai = f64(A, 2 * k + 1);
    const br = f64(B, 2 * k);
    const bi = f64(B, 2 * k + 1);
  ...
  for (let i = 0; i < length; i++) out[i] = f64(C, 2 * (offset + i));
  return toHostTensor(out, [length], dtype, device);
}

export function convolve(a: NumericVectorInput, b: NumericVectorInput, opts: ModeOptions = {}): Tensor {
  const av = hostVector(a);
  const bv = hostVector(b);
  return fftConvolve(av.data, bv.data, opts.mode ?? 'full', av.device, av.dtype);
}

export function correlate(a: NumericVectorInput, b: NumericVectorInput, opts: ModeOptions = {}): Tensor {
  const av = hostVector(a);
  const bv = hostVector(b);
...
function checkWindow(window: number, n: number): number {
...
export function rollingSum(x: NumericVectorInput, window: number): Tensor {
...
export function rollingMean(x: NumericVectorInput, window: number): Tensor {
  const { data, device, dtype } = hostVector(x);
  const n = data.length;
  const w = checkWindow(window, n);
  const prefix = new Float64Array(n + 1);
...
export function rollingStd(x: NumericVectorInput, window: number, opts: RollingOptions = {}): Tensor {
  const { data, device, dtype } = hostVector(x);
  const n = data.length;
  const w = checkWindow(window, n);
...
function rollingExtreme(x: NumericVectorInput, window: number, keep: (a: number, b: number) => boolean): Tensor {
  const { data, device, dtype } = hostVector(x);
  const n = data.length;
  const w = checkWindow(window, n);
  const out = new Float64Array(n - w + 1);
...
export function rollingMin(x: NumericVectorInput, window: number): Tensor {
  return rollingExtreme(x, window, (a: number, b: number) => a <= b);
}

export function rollingMax(x: NumericVectorInput, window: number): Tensor {
  return rollingExtreme(x, window, (a: number, b: number) => a >= b);
}

export function polyfit(x: NumericVectorInput, y: NumericVectorInput, deg: number): Tensor {
  ...
  return toHostTensor(float64From(tensorToContiguous(sol)), [cols], 'f64', xv.device);
}

function horner(c: Float64Array, x: number): number {
  let acc = 0;
  for (let i = 0; i < c.length; i++) acc = acc * x + c[i];
  return acc;
}

export function polyval(coeffs: NumericVectorInput, x: number): number;
export function polyval(coeffs: NumericVectorInput, x: NumericArrayInput): Tensor;
export function polyval(coeffs: NumericVectorInput, x: number | NumericArrayInput): number | Tensor {
...
export function polyroots(coeffs: NumericVectorInput): Tensor {
