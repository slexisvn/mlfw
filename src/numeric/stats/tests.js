...
function tPvalue(t, df) {
  return clampP(2 * (1 - studentT.cdf(Math.abs(t), df)));
}

function chi2Pvalue(stat, df) {
  return clampP(1 - chi2Dist.cdf(stat, df));
}

export function tTest1Samp(x, opts = {}) {
...
export function tTestInd(x, y, opts = {}) {
...
export function tTestPaired(x, y, opts = {}) {
...
export function chi2Gof(observed, expected, opts = {}) {
...
export function ksTest2Samp(x, y) {
...
export function andersonDarling(x) {
  const { data } = hostVector(x);
  const sorted = Float64Array.from(data).sort();
  const n = sorted.length;
