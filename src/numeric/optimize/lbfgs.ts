...
import type { Bounds, GradientFn, VectorFn } from '../types.js';
...
function twoLoop(g: readonly number[], sList: readonly number[][], yList: readonly number[][], rhoList: readonly number[]): number[] {
  const q = g.slice();
...
function minimize(f: VectorFn, x0: readonly number[], opts: LbfgsOptions, bounds: Bounds | null): MinimizeResult {
  const n = x0.length;
  ...
  const projectGrad = (grad: number[], at: readonly number[]): number[] => {
...
export function lbfgs(f: VectorFn, x0: readonly number[], opts: LbfgsOptions = {}): MinimizeResult {
  return minimize(f, x0, opts, null);
}

export function lbfgsB(f: VectorFn, x0: readonly number[], bounds: Bounds, opts: LbfgsOptions = {}): MinimizeResult {
