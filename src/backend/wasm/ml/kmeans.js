...
function assignWasm(mod, xp, cp, lp, ip, n, d, k, centers, labels) {
...
export function wasmKmeans(_keySet, X, nClusters, maxIter, nInit, seed) {
  ...
  for (let i = 0; i < n; i++) labelsOut[i] = best.labels[i];
  return [
...
export function wasmKmeansPredict(_keySet, X, centersT) {
  const { data, rows: n, cols: d } = hostMatrix(X);
