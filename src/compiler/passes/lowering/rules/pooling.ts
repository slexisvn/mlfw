...
import { registerLoweringRule, buildSpatialNest, parseLayout, markCommReduce } from '../lowering_registry.js';
import type { TirNode } from '../../../ir/tensor/nodes.js';

function spatialCount(ov: TirNode, stride: number, kernel: number, pad: number, inExtent: number): TirNode {
...
function avgPoolDivisorExpr(ohv: TirNode, owv: TirNode, sH: number, sW: number, kH: number, kW: number, padH: number, padW: number, inH: number, inW: number, outDtype: string): TirNode {
...
export function register(): void {
  registerLoweringRule('pool2d', (ctx, op, inputs, outputs) => {
    ...
    const poolType = op.getAttr<string>('pool_type');
    const kernelSize = op.getAttr<readonly number[]>('kernel_size') as readonly number[];
    const strides = op.getAttr<readonly number[]>('strides') as readonly number[];
    const padding = op.getAttr<readonly (readonly number[])[]>('padding') as readonly (readonly number[])[];
    const countIncludePad = op.getAttr<boolean>('count_include_pad') || false;
    const layout = parseLayout(op.getAttr<string>('layout') || 'NCHW');
    const nAx = layout['N'];
    const cAx = layout['C'];
    const hAx = layout['H'];
    const wAx = layout['W'];
    ...
    const outH = outBuf.shape[hAx] as number;
    const outW = outBuf.shape[wAx] as number;
    ...
    markCommReduce(allBinds.slice(4));
    ...
    const outIdx: TirNode[] = new Array(4);
    outIdx[nAx] = nv; outIdx[cAx] = cv; outIdx[hAx] = ohv; outIdx[wAx] = owv;
    const inIdx: TirNode[] = new Array(4);
