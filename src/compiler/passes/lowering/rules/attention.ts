...
type AttentionDims = {
  Q: Buffer; K: Buffer; V: Buffer; O: Buffer | null;
  scale: number; causal: boolean; dtype: string;
  B: number; H: number; Lq: number; Dk: number; Lk: number; Dv: number;
};

const Z = new IntImmNode(0);
const I = (n: number): IntImmNode => new IntImmNode(n);
...
function dims(op: Operation, inputs: readonly Buffer[]): AttentionDims {
  const [Q, K, V] = inputs;
  return {
    Q, K, V, O: null,
    scale: op.getAttr<number>('scale') as number,
    causal: !!op.getAttr<boolean>('causal'),
    dtype: Q.dtype,
    B: Q.shape[0] as number, H: Q.shape[1] as number, Lq: Q.shape[2] as number, Dk: Q.shape[3] as number,
    Lk: K.shape[2] as number, Dv: V.shape[3] as number,
  };
}

function buildNaive(ctx: LoweringContext, op: Operation, inputs: Buffer[], outputs: Buffer[]): TirNode {
  ...
  let perQuery: TirNode = new SeqNode([initM, initL, initO, jLoop, finalW]);
...
function buildTiled(ctx: LoweringContext, op: Operation, inputs: Buffer[], outputs: Buffer[], TPB: number): TirNode {
  const { Q, K, V, scale, causal, dtype, B, H, Lq, Dk, Lk, Dv } = dims(op, inputs);
  const O = outputs[0];
  ...
  let perThread: TirNode = new LetStmtNode(i, iExpr, new SeqNode([prologue, ktLoop, writeBack]));
