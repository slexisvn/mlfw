...
export type RematPolicyOpts = Readonly<{
  maxRematDepth?: number;
  sizeThreshold?: number;
  alwaysRemat?: readonly string[];
  neverRemat?: readonly string[];
}>;

export class RematPolicy {
  private _maxRematDepth: number;
  private _sizeThreshold: number;
  private _alwaysRemat: Set<string>;
  private _neverRemat: Set<string>;

  constructor(opts: RematPolicyOpts = {}) {
  ...
  _isElementwise(op: Operation): boolean {
    if (op.numOperands === 0 || op.numResults === 0) return false;
    const resultShape = (op.getResult(0).type as TensorType).shape;
    for (let i = 0; i < op.numOperands; i++) {
      const operandType = op.getOperand(i).type as TensorType;
