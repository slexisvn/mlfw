...
registerVJPRule('conv', (ctx) => {
  ...
  const fullPad = [
    [(KH as number) - 1 - padH[0], (KH as number) - 1 - padH[1]],
    [(KW as number) - 1 - padW[0], (KW as number) - 1 - padW[1]]
  ];
  const dInput = b.conv(grad, wFlip, [1, 1], fullPad as unknown as readonly number[]).getResult(0);

  const xSwap = b.transpose(input, [1, 0, 2, 3]).getResult(0);
  const gSwap = b.transpose(grad, [1, 0, 2, 3]).getResult(0);
  const dWconv = b.conv(xSwap, gSwap, [1, 1], [padH, padW] as unknown as readonly number[]).getResult(0);
