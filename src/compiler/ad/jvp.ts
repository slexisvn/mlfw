...
export type JVPContext = {
  builder: IRBuilder;
  op: Operation;
  operands: Value[];
  operandTangents: Value[];
  results: Value[];
  attrs: ReadonlyMap<string, AttrValue>;
};

export type JVPRule = (ctx: JVPContext) => Value | (Value | null)[] | null;

const _jvpRules = new Map<string, JVPRule>();

export function registerJVPRule(opName: string, fn: JVPRule): void {
  _jvpRules.set(opName, fn);
}

export function getJVPRule(opName: string): JVPRule | null {
  return _jvpRules.get(opName) || null;
}

export function buildForwardDiff(forwardFunc: GraphFunction): GraphFunction {
  ...
  const argTypes = inputs.map((v: Value) => v.type);

  const jvpFunc = new GraphFunction(`jvp_${forwardFunc.name}`, [...argTypes, ...argTypes], outputs.map((v: Value) => v.type));
  const builder = new IRBuilder(jvpFunc);
  const jvpArgs = jvpFunc.args;

  const fwdMap = new Map<Value, Value>();
  const tan = new Map<Value, Value>();
  for (let i = 0; i < inputs.length; i++) {
    fwdMap.set(inputs[i], jvpArgs[i]);
    tan.set(inputs[i], jvpArgs[inputs.length + i]);
  }

  const zeroLike = (type: TensorType): Value => builder.broadcast(builder.scalarConstant(0, type.dtype).getResult(0), type.shape, []).getResult(0);
  ...
  for (const op of topo) {
    ...
    if (isConstantOp(op.opName)) {
      for (let r = 0; r < op.numResults; r++) tan.set(op.getResult(r), zeroLike(op.getResult(r).type as TensorType));
      continue;
    }
    ...
    const operands = op.operands.map((o: Value) => fwdMap.get(o) || o);
    const operandTangents = op.operands.map((o: Value) => tan.get(o) || zeroLike(o.type as TensorType));
    ...
    for (let r = 0; r < op.numResults; r++) {
      tan.set(op.getResult(r), dys[r] != null ? dys[r] as Value : zeroLike(op.getResult(r).type as TensorType));
    }
  }

  builder.returnOp(outputs.map((o: Value) => tan.get(o) || zeroLike(o.type as TensorType)));
  return jvpFunc;
}

const g = (x: Operation): Value => x.getResult(0);

registerJVPRule('add', (ctx: JVPContext) => g(ctx.builder.add(ctx.operandTangents[0], ctx.operandTangents[1])));
registerJVPRule('sub', (ctx: JVPContext) => g(ctx.builder.sub(ctx.operandTangents[0], ctx.operandTangents[1])));
registerJVPRule('neg', (ctx: JVPContext) => g(ctx.builder.neg(ctx.operandTangents[0])));
registerJVPRule('mul', (ctx: JVPContext) => {
...
registerJVPRule('div', (ctx: JVPContext) => {
  const [, b] = ctx.operands;
  const [da, db] = ctx.operandTangents;
  const y = ctx.results[0];
  return g(ctx.builder.div(g(ctx.builder.sub(da, g(ctx.builder.mul(y, db)))), b));
});
registerJVPRule('exp', (ctx: JVPContext) => g(ctx.builder.mul(ctx.results[0], ctx.operandTangents[0])));
registerJVPRule('log', (ctx: JVPContext) => g(ctx.builder.div(ctx.operandTangents[0], ctx.operands[0])));
registerJVPRule('sqrt', (ctx: JVPContext) => {
  const da = ctx.operandTangents[0];
  const y = ctx.results[0];
  const two = g(ctx.builder.broadcast(g(ctx.builder.scalarConstant(2, (y.type as TensorType).dtype)), (y.type as TensorType).shape, []));
  return g(ctx.builder.div(da, g(ctx.builder.mul(two, y))));
});
registerJVPRule('tanh', (ctx: JVPContext) => {
  const da = ctx.operandTangents[0];
  const y = ctx.results[0];
  const one = g(ctx.builder.broadcast(g(ctx.builder.scalarConstant(1, (y.type as TensorType).dtype)), (y.type as TensorType).shape, []));
  const oneMinusSq = g(ctx.builder.sub(one, g(ctx.builder.mul(y, y))));
  return g(ctx.builder.mul(oneMinusSq, da));
});

function fillLike(ctx: JVPContext, type: TensorType, value: number): Value {
  return g(ctx.builder.broadcast(g(ctx.builder.scalarConstant(value, type.dtype)), type.shape, []));
}

function cloneToTangent(ctx: JVPContext, valueForOperand: (i: number) => Value): Operation {
...
function linearJVP(ctx: JVPContext): Value | Value[] {
  const cloned = cloneToTangent(ctx, (i: number) => ctx.operandTangents[i]);
  if (cloned.numResults === 1) return cloned.getResult(0);
  return cloned.results.map((_: Value, i: number) => cloned.getResult(i));
}

function linearJVPKeep(ctx: JVPContext, keep: ReadonlySet<number>): Value {
  return cloneToTangent(ctx, (i: number) => keep.has(i) ? ctx.operands[i] : ctx.operandTangents[i]).getResult(0);
}

function bilinearJVP(ctx: JVPContext): Value {
...
for (const name of ZERO_OPS) registerJVPRule(name, (ctx) => ctx.results.map((r) => fillLike(ctx, r.type as TensorType, 0)));
...
registerJVPRule('convert', (ctx) => {
  const target = ctx.op.getAttr('target_dtype') || (ctx.results[0].type as TensorType).dtype;
  if (target === 'i8' || target === 'i16' || target === 'i32' || target === 'i64' || target === 'ui8' || target === 'bool') {
    return fillLike(ctx, ctx.results[0].type as TensorType, 0);
  }
...
registerJVPRule('square', (ctx) => {
  const two = fillLike(ctx, ctx.operands[0].type as TensorType, 2);
...
registerJVPRule('rsqrt', (ctx) => {
  const y = ctx.results[0];
  const half = fillLike(ctx, y.type as TensorType, -0.5);
...
registerJVPRule('sigmoid', (ctx) => {
  const y = ctx.results[0];
  const one = fillLike(ctx, y.type as TensorType, 1);
...
registerJVPRule('pow', (ctx) => {
  ...
  const [da, db] = ctx.operandTangents;
  const y = ctx.results[0];
  const one = fillLike(ctx, a.type as TensorType, 1);
