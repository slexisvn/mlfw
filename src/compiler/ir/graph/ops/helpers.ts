...
export function inferBinaryElementwise(operandTypes: readonly IRType[]): IRType[] | null {
...
export function inferUnaryElementwise(operandTypes: readonly IRType[]): IRType[] | null {
...
export function inferCompare(operandTypes: readonly IRType[]): IRType[] | null {
  if (operandTypes.length !== 2) return null;
  const lhs = operandTypes[0], rhs = operandTypes[1];
  if (!(lhs instanceof TensorType) || !(rhs instanceof TensorType)) return null;
  const shape = TensorType.broadcastShape(lhs.shape, rhs.shape);
...
export function inferUnaryFloat(operandTypes: readonly IRType[]): IRType[] | null {
  if (operandTypes.length !== 1) return null;
...
export function verifyBinaryElementwise(op: Operation): string[] {
  const errs: string[] = [];
...
export function verifyUnaryElementwise(op: Operation): string[] {
  const errs: string[] = [];
...
export function verifyUnaryFloat(op: Operation): string[] {
  const errs = verifyUnaryElementwise(op);
  if (errs.length === 0) {
    const dt = (op.getOperand(0).type as TensorType).dtype;
