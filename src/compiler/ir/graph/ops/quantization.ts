...
const QUANTIZED_INT_DTYPES = new Set<ScalarDType>([ScalarType.I8, ScalarType.UI8]);
const DEQUANT_INPUT_DTYPES = new Set<ScalarDType>([ScalarType.I8, ScalarType.UI8, ScalarType.I32]);
...
export function register(registry: OpRegistry) {
  registry.register(new OpDef({
    ...
    verify(op) {
      ...
      const tgt = op.getAttr<ScalarDType>('target_dtype')!;
    ...
    fold(constValues, attrs) {
      ...
      const scale = attrs.get ? attrs.get('scale') : (attrs as unknown as OpAttrRecord).scale;
      const zp = attrs.get ? attrs.get('zero_point') : (attrs as unknown as OpAttrRecord).zero_point;
