...
import type { VarRange } from './iter_map.js';
import type { IRNode } from '../ir/ir_visitor.js';
import type { TirNode, BlockNode, IntImmNode } from '../ir/tensor/nodes.js';
...
export type BufferAccessEnv = {
  loopRanges: ReadonlyMap<string, VarRange>;
  varForms: ReadonlyMap<string, LinearForm>;
  iterSpace: readonly IterLevel[];
} | null;
...
export class BufferAccess {
  node: TirNode;
  buffer: Buffer;
  kind: AccessKindValue;
  position: number;
  conditional: boolean;
  regions: AccessRegion[];
  selfReferential: boolean;
  forms: (LinearForm | null)[];
  iterSpace: IterLevel[];
  block: BlockNode | null;
  indexBuffers: Set<Buffer>;

  constructor(node: TirNode, buffer: Buffer, kind: AccessKindValue, position: number, conditional: boolean, regions: AccessRegion[], selfReferential: boolean, forms: (LinearForm | null)[], iterSpace: IterLevel[], block: BlockNode | null, indexBuffers: Set<Buffer>) {
    this.node = node;
    this.buffer = buffer;
    this.kind = kind;
  ...
  get isWrite(): boolean { return this.kind === AccessKind.WRITE; }
  get isRead(): boolean { return this.kind === AccessKind.READ; }
}
...
export class BlockAccessInfo {
  ...
  typedIterVars: boolean;
  directBodyVars: Set<string>;
  accesses: BufferAccess[];

  constructor(block: BlockNode, iterSpace: IterLevel[], bindings: IterBinding[], affineBinding: boolean, typedIterVars: boolean, directBodyVars: Set<string>) {
    this.block = block;
    this.iterSpace = iterSpace;
...
function directBodyVarsOf(block: BlockNode, own: ReadonlySet<string>): Set<string> {
  const vars = new Set<string>();
  const visit = (n: IRNode): void => { if (n.type === 'VariableNode' && !own.has(n.name)) vars.add(n.name); };
  walk(block.body, visit);
  if (block.initBody) walk(block.initBody, visit);
  return vars;
}

function constValue(node: TirNode | null | undefined): number | null {
  return node && node.type === 'IntImmNode' && Number.isInteger((node as IntImmNode).value) ? (node as IntImmNode).value : null;
}

export function loadedBuffers(exprs: readonly (TirNode | null | undefined)[]): Set<Buffer> {
  const found = new Set<Buffer>();
  for (const expr of exprs) {
    if (expr) walk(expr, (n: IRNode): void => { if (n.type === 'BufferLoadNode' && n.buffer) found.add(n.buffer); });
  }
  return found;
}

export function collectBufferAccesses(root: TirNode, env: BufferAccessEnv = null): BufferAccessResult {
  const order: BufferAccess[] = [];
  ...
  const bind = (name: string, range: VarRange | null, form: LinearForm | null): void => {
  ...
  const openScope = (): number => scopes.push([]);
  const closeScope = (): void => {
    for (const [name, prevRange, prevForm] of scopes.pop() as [string, VarRange | undefined, LinearForm | undefined][]) {
  ...
  const record = (node: TirNode, buffer: Buffer, kind: AccessKindValue, indices: readonly TirNode[], valueLoads: Set<Buffer> | null): BufferAccess => {
    const regions: AccessRegion[] = [];
    const forms: (LinearForm | null)[] = [];
    ...
    list.push(access);
    if (block) (byBlock.get(block) as BlockAccessInfo).accesses.push(access);
    return access;
  };

  walk(root, {
    pre(node: IRNode): void {
      switch (node.type) {
        case 'ForNode': {
          openScope();
