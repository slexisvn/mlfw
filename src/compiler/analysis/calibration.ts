...
export type CalibrationMode = 'minmax' | 'percentile' | 'entropy';
...
export class ValueObserver {
  ...
  max: number;
  count: number;
  histogram: HistogramCollector | null;
  ...
  enableHistogram(numBins = 2048): void {
...
export class HistogramCollector {
  ...
  _initRange(data: ArrayLike<number>): void {
  ...
  _expandAndRebucket(newData: ArrayLike<number>): void {
  ...
  computePercentileThreshold(percentile: number): number {
  ...
  computeEntropyThreshold(numQuantBins: number): number {
...
export class CalibrationCollector {
  observers: Map<Value, ValueObserver>;
  mode: string;

  constructor(mode: string = 'minmax') {
    this.observers = new Map();
    this.mode = mode;
  }

  attach(func: GraphFunction): void {
  ...
  observe(value: Value, data: ArrayLike<number>): void {
...
export class CalibrationResult {
  private _observers: Map<Value, ValueObserver>;
  private _mode: string;

  constructor(observers: Map<Value, ValueObserver>, mode: string) {
    this._observers = observers;
    this._mode = mode;
  }

  getRange(value: Value): ValueRange | null {
    const obs = this._observers.get(value);
    if (!obs || obs.count === 0) return null;
    return { min: obs.min, max: obs.max };
  }

  getQuantParams(value: Value, scheme: QuantizationSchemeValue, dtype: ScalarDType): QuantizationParams | null {
    const range = this.getRange(value);
    if (!range) return null;

    const asymmetric = scheme === QuantizationScheme.PER_TENSOR_ASYMMETRIC;

    if (this._mode === 'percentile' && (this._observers.get(value) as ValueObserver).histogram) {
      if (asymmetric) return QuantizationParams.fromRange(range.min, range.max, scheme, dtype);
      const hist = (this._observers.get(value) as ValueObserver).histogram as HistogramCollector;
      const hi = hist.computePercentileThreshold(0.999);
      return QuantizationParams.fromRange(-hi, hi, scheme, dtype);
    }

    if (this._mode === 'entropy' && (this._observers.get(value) as ValueObserver).histogram) {
      if (asymmetric) return QuantizationParams.fromRange(range.min, range.max, scheme, dtype);
      const hist = (this._observers.get(value) as ValueObserver).histogram as HistogramCollector;
