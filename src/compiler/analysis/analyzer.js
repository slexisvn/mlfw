...
export class Analyzer {
  ...
  constIntBound(expr) {
    ...
    if (expr.type === 'var') {
      return this._varBounds.get(expr.name) || EVERYTHING;
    }

    const a = expr.args.length > 0 ? this.constIntBound(expr.args[0]) : null;
    const b = expr.args.length > 1 ? this.constIntBound(expr.args[1]) : null;
  ...
  canProveGreaterEqual(expr, value) {
    const bound = this.constIntBound(SymInt.sub(expr, value));
    return bound.min >= 0;
  }

  canProveLess(expr, value) {
    const bound = this.constIntBound(SymInt.sub(expr, value));
    return bound.max < 0;
  }

  canProveNonNegative(expr) {
