...
function gpuThreadCap(target: ScheduleTarget | null | undefined): number {
  return Math.min((target && target.maxThreadsPerBlock) || 256, 256);
}

function reductionDivisors(K: number): number[] {
...
export function createFusedTilingSketch(consumerName: string): ScheduleSketch {
...
export function createRfactorSketch(blockInfo: BlockClassification): ScheduleSketch | null {
  const reductionLoops = blockInfo.loops.filter((l: ForNode) => blockInfo.reductionLoopVars.has(l.loopVar.name));
  if (reductionLoops.length !== 1) return null;
  const kLoop = reductionLoops[0];
  const K = kLoop.extent && kLoop.extent.type === 'IntImmNode' ? (kLoop.extent as IntImmNode).value : null;
...
export function createElementwiseCPUSketch(): ScheduleSketch {
  return new ScheduleSketch('elementwise_cpu', [
    ...
    if (loops.length === 1) {
      ...
      if (extent.type === 'IntImmNode' && extent.value >= (params.vector_width as number) * 2) {
        const [outer, inner] = schedule.split(loops[0], (params.vector_width as number));
    ...
    if (extent.type === 'IntImmNode' && extent.value >= (params.vector_width as number)) {
      const [, vi] = schedule.split(innermost, (params.vector_width as number));
      schedule.vectorize(vi);
    }
  });
}

export function createElementwiseGPUSketch(): ScheduleSketch {
  return new ScheduleSketch('elementwise_gpu', [
    ...
    const blockSize = Math.min((params.block_size as number), gpuThreadCap(target));
...
export function createReductionCPUSketch(): ScheduleSketch {
...
export function createReductionGPUSketch(): ScheduleSketch {
