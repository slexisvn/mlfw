...
export type ConvInfo = {
  N: number; Cin: number; H: number; W: number;
  O: number; Kh: number; Kw: number; Oh: number; Ow: number;
  sH: number; sW: number; pH: number; pW: number; dH: number; dW: number;
};
export type ConvBuffers = { weight: Buffer; input: Buffer; output: Buffer };
export type GemmTileConfig = { BM: number; BN: number; BK: number; TM: number; TN: number };
export type GemmDims = { M: number; N: number; K: number };
export type ConvScheduleConfig = Readonly<{ convConfig?: GemmTileConfig; convNoVec?: boolean; primitiveMatmul?: boolean }>;
...
const SUB = (a: TirNode, b: TirNode): MathOpNode => new MathOpNode('-', a, b);
const MUL = (a: TirNode, b: TirNode): MathOpNode => new MathOpNode('*', a, b);
const DIV = (a: TirNode, b: TirNode): MathOpNode => new MathOpNode('//', a, b);
const MOD = (a: TirNode, b: TirNode): MathOpNode => new MathOpNode('%', a, b);
const LT = (a: TirNode, b: TirNode): CompareNode => new CompareNode('lt', a, b);
const GE = (a: TirNode, b: TirNode): CompareNode => new CompareNode('ge', a, b);
const AND = (a: TirNode, b: TirNode): MathOpNode => new MathOpNode('&&', a, b);
const forS = (v: VariableNode, e: number, b: TirNode): ForNode => new ForNode(v, I(0), I(e), ForKind.SERIAL, b);
const forU = (v: VariableNode, e: number, b: TirNode): ForNode => new ForNode(v, I(0), I(e), ForKind.UNROLLED, b);
const forT = (v: VariableNode, t: string, e: number, b: TirNode): ForNode => new ForNode(v, I(0), I(e), ForKind.THREAD_BINDING, b, t);

export function detectPureConv(graphFunc: GraphFunction): ConvInfo | null {
  let conv: Operation | null = null;
  for (const op of graphFunc.ops()) {
    if (op.opName === 'conv') { if (conv) return null; conv = op; }
    else if (op.opName !== 'return') return null;
  }
  if (!conv) return null;
  if ((conv.getAttr<number>('groups') || 1) !== 1) return null;
  const il = conv.getAttr<string>('input_layout');
  const kl = conv.getAttr<string>('kernel_layout');
  if ((il && il !== 'NCHW') || (kl && kl !== 'OIHW')) return null;
  const inT = conv.getOperand(0).type as TensorType, kerT = conv.getOperand(1).type as TensorType, outT = conv.getResult(0).type as TensorType;
  ...
  const strides = conv.getAttr<readonly number[]>('strides') || [1, 1];
  const pad = conv.getAttr<readonly (readonly number[])[]>('padding') || [[0, 0], [0, 0]];
  const dil = conv.getAttr<readonly number[]>('dilation') || [1, 1];
...
export function buildImplicitGemmConv(bufs: ConvBuffers, ci: ConvInfo, params: GemmTileConfig): TirNode {
  ...
  const accIdx = (mi: TirNode, ni: TirNode): MathOpNode => ADD(MUL(mi, I(TN)), ni);
  ...
  let aVal: TirNode = new BufferLoadNode(weight, [aM, aCin, aKh, aKw]);
  if (guardRow || guardK) {
    let c: TirNode | null = guardRow ? LT(aM, I(M)) : null;
    if (guardK) c = c ? AND(c, LT(aK, I(K))) : LT(aK, I(K));
    aVal = new IfThenElseNode(c as TirNode, aVal, FZERO());
  }
  ...
  let bVal: TirNode = new BufferLoadNode(input, [nB, bCin, ih, iw]);
  let inb: TirNode | null = AND(AND(GE(ih, I(0)), LT(ih, I(H))), AND(GE(iw, I(0)), LT(iw, I(W))));
  if (guardK) inb = AND(inb, LT(bK, I(K)));
  if (guardCol) inb = AND(inb, LT(bN, I(GN)));
  bVal = new IfThenElseNode(inb as TirNode, bVal, FZERO());
  ...
  let writeStore: TirNode = new BufferStoreNode(output, [wB, cM, wOh, wOw], new BufferLoadNode(acc, [accIdx(wm, wn)]));
  if (guardRow || guardCol) {
    let c: TirNode | null = guardRow ? LT(cM, I(M)) : null;
    if (guardCol) c = c ? AND(c, LT(cN, I(GN))) : LT(cN, I(GN));
    writeStore = new IfThenElseNode(c as TirNode, writeStore);
  }
...
export function vectorizableConvConfig(target: ScheduleTarget, ci: ConvInfo, cfg: GemmTileConfig): boolean {
...
export function buildVectorizedImplicitGemmConv(bufs: ConvBuffers, ci: ConvInfo, params: GemmTileConfig): TirNode {
  const { weight, input, output } = bufs;
  const { N, Cin, H, W, O, Kh, Kw, Oh, Ow, sH, sW, pH, pW, dH, dW } = ci;
  const { BM, BN, BK, TM, TN } = params;
  const V = VEC;
  ...
  const prefetch = (k0e: TirNode): TirNode => {
    const u = uid++;
    ...
    let v: TirNode = new BufferLoadNode(inputFlat, [off]);
    let inb: TirNode | null = null;
    if (needGuardH) inb = AND(GE(ih, I(0)), LT(ih, I(H)));
    if (needGuardW) { const cc = AND(GE(iw, I(0)), LT(iw, I(W))); inb = inb ? AND(inb, cc) : cc; }
    if (inb) v = new IfThenElseNode(inb as TirNode, v, FZERO());
  ...
  const commit = (offAe: TirNode, offBe: TirNode): TirNode => {
    const u = uid++;
    const la = IV('iv_la' + u), c = IV('iv_c' + u);
    const m = DIV(c, I(BKv)), kc = MUL(MOD(c, I(BKv)), I(V));
    const stores = [];
  ...
  const computeMMA = (offAe: TirNode, offBe: TirNode): TirNode => {
    const u = uid++;
    const kk = IV('iv_kk' + u);
    const frags = [];
...
function validConvConfig(target: ScheduleTarget, dims: GemmDims, cfg: GemmTileConfig): boolean {
  const { BM, BN, BK, TM, TN } = cfg;
...
const VEC_CONFIGS: readonly GemmTileConfig[] = [
...
function pickVectorizedConvConfig(target: ScheduleTarget, ci: ConvInfo): GemmTileConfig | null {
...
export function applyImplicitGemmConv(schedule: Schedule, target: ScheduleTarget, sCfg?: ConvScheduleConfig): boolean {
  const pf = schedule.func;
  const ci = (pf.getAttr as (k: string) => unknown)(FuncAttr.CONV_INFO) as ConvInfo | null;
  if (!ci) return false;
