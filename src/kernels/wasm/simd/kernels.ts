...
type WatDecl = readonly [name: string, type: string];
type DistTemps = { j: string; nmain: string; acc: string; va: string; vb: string; diff: string };
type DotTemps = { j: string; nmain: string; acc: string };
type AxpyTemps = { j: string; nmain: string; sv: string; ya: string; vy: string };

const DIST_SENTINEL = 1e308;
const MEM_MIN_PAGES = 1;
const MEM_MAX_PAGES = 32768;

function moduleWrap(exportName: string, params: readonly WatDecl[], locals: readonly WatDecl[], body: string): string {
...
function sqDistSnippet(tag: string, xrow: string, crow: string, d: string, out: string, t: DistTemps): string {
  return `
...
const _DIST_LOCALS: readonly WatDecl[] = [
  ['j', 'i32'], ['nmain', 'i32'], ['acc', 'v128'], ['va', 'v128'], ['vb', 'v128'], ['diff', 'f64'],
];

export function nearestCentroidWat(): string {
  const body = `
...
export function distRowWat(): string {
  const body = `
...
function reduceDotSnippet(tag: string, a: string, b: string, n: string, out: string, t: DotTemps): string {
  return `
...
function axpySnippet(tag: string, y: string, x: string, scalar: string, n: string, t: AxpyTemps): string {
  return `
...
export function gramSymWat(): string {
...
export function matmulRowsWat(): string {
  const dot = { j: 'kk', nmain: 'nm', acc: 'acc' };
  const body = `
    (i32.const 0) local.set $i
    (block $ie (loop $il
...
export function coordDescentWat(): string {
