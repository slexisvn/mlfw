...
function chooseFeatures(d, maxFeatures, rng) {
...
function bestSplitClassify(x, d, indices, feats, K, labels, minLeaf) {
...
function bestSplitRegress(x, d, indices, feats, yData, minLeaf) {
  const m = indices.length;
  let best = null;
  for (const f of feats) {
    const order = indices.slice().sort((a, b) => x[a * d + f] - x[b * d + f]);
...
export function cpuDecisionTreeFit(_keySet, X, y, maxDepth, minSplit, minLeaf, maxFeatures, classify, seed) {
  ...
  const isPure = (indices) => {
    const ref = classify ? labels : yv.data;
  ...
  const build = (indices, depth) => {
    ...
    const split = classify
      ? bestSplitClassify(x, d, indices, feats, K, labels, minLeaf)
...
export function cpuDecisionTreePredict(_keySet, X, featureT, thresholdT, leftT, rightT, valueT) {
  const m = hostMatrix(X);
