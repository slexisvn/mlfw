...
function stackTensors(tensors, dim = 0) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (src.isContiguous) {
      outData.set(srcData.subarray(srcOffset, srcOffset + elemNumel), i * elemNumel);
    } else {
      ...
      for (let j = 0; j < elemNumel; j++) {
        outData[i * elemNumel + j] = storage[si];
...
export function defaultCollate(batch) {
  const elem = batch[0];

  if (elem instanceof Tensor) {
    return stackTensors(batch, 0);
  }
  ...
  if (Array.isArray(elem)) {
    ...
    for (let i = 0; i < elem.length; i++) {
      ...
      for (let j = 0; j < batch.length; j++) column[j] = batch[j][i];
  ...
  if (elem !== null && typeof elem === 'object' && elem.constructor === Object) {
    ...
    for (let k = 0; k < keys.length; k++) {
      const key = keys[k];
      const column = new Array(batch.length);
      for (let j = 0; j < batch.length; j++) column[j] = batch[j][key];
