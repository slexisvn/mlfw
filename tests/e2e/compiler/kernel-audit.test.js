...
describe('Kernel audit — print kernel sources for manual review', () => {
  it('dumps kernel sources for key patterns', () => {
    const models = [];

    {
      const t = new TensorType([64], ScalarType.F32);
      const func = buildFunction('ew_chain', [t], [t], (b, args) => {
        let v = b.sigmoid(args[0]).getResult(0);
        v = b.mul(v, args[0]).getResult(0);
        v = b.tanh(v).getResult(0);
        b.returnOp([v]);
      });
      const r = compile(func);
      models.push({ name: 'ew_chain (sigmoid->mul->tanh)', src: r.getSource('ew_chain') });
    }

    {
      ...
      const w = new TensorType([8, 4], ScalarType.F32);
      const out = new TensorType([4, 4], ScalarType.F32);
      const func = buildFunction('matmul_only', [x, w], [out], (b, args) => {
        b.returnOp([b.matmul(args[0], args[1]).getResult(0)]);
      });
      const r = compile(func);
      models.push({ name: 'matmul [4,8]x[8,4]', src: r.getSource('matmul_only') });
    }

    {
      ...
      const r = compile(func);
      models.push({ name: 'conv3x3 pad=0', src: r.getSource('conv_nopad') });
    }

    {
      ...
      const g = new TensorType([8], ScalarType.F32);
      const beta = new TensorType([8], ScalarType.F32);
      const func = buildFunction('ln_dump', [t, g, beta], [t], (b, args) => {
        b.returnOp([b.layernorm(args[0], args[1], args[2]).getResult(0)]);
      });
      const r = compile(func);
      models.push({ name: 'layernorm [4,8]', src: r.getSource('ln_dump') });
    }

    {
      ...
      const var_ = new TensorType([8], ScalarType.F32);
      const out = new TensorType([1, 8, 6, 6], ScalarType.F32);
      const func = buildFunction('conv_bn_relu_dump', [x, k, g, beta, mean, var_], [out], (b, args) => {
        const conv = b.conv(args[0], args[1], [1, 1], [[0, 0], [0, 0]]).getResult(0);
        const bn = b.batchnorm(conv, args[2], args[3], args[4], args[5]).getResult(0);
        b.returnOp([b.relu(bn).getResult(0)]);
      });
      const r = compile(func);
      models.push({ name: 'conv+BN+relu', src: r.getSource('conv_bn_relu_dump') });
    }

    {
      ...
      const out = new TensorType([4, 8], ScalarType.F32);
      const func = buildFunction('attn_dump', [Q, K, V], [out], (b, args) => {
        ...
        const attended = b.matmul(attn, args[2]).getResult(0);
        b.returnOp([attended]);
      });
      const r = compile(func);
      models.push({ name: 'self-attention', src: r.getSource('attn_dump') });
    }

    {
      ...
      const func = buildFunction('swiglu_dump', [x, wGate, wUp, gamma, beta], [out], (b, args) => {
      ...
      const r = compile(func);
      models.push({ name: 'SwiGLU+layernorm', src: r.getSource('swiglu_dump') });
    }

    for (const m of models) {
      const au = audit(m.src, m.name);
      console.log(`\n=== ${m.name} ===`);
      console.log(`  loops: ${au.loops}, temps: ${au.tempBuffers}, lines: ${au.lines}`);
      console.log(`  noise: [${au.arithmeticNoise.join(', ')}], extent1: ${au.extent1Loops}`);
      console.log(`  zeroInits: ${au.zeroInits}, boundsChecks: ${au.boundsChecks}`);
      console.log(`  modulos: ${au.modulos}, truncs: ${au.truncs}`);
      console.log('---SOURCE---');
      console.log(m.src);
      console.log('---END---');
