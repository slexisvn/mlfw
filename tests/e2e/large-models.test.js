...
function nest(fl, s) { if (s.length === 1) return fl.slice(0, s[0]); const sub = numel(s.slice(1)); return Array.from({ length: s[0] }, (_, i) => nest(fl.slice(i * sub, (i + 1) * sub), s.slice(1))); }
function mkInput(rng, shape) { const n = numel(shape); const f = []; for (let i = 0; i < n; i++) f.push(-1 + 2 * rng()); return tensor(nest(f, shape)); }
...
async function expectMatchesEager(fwd, inputs, tol = 2e-3) {
  ...
  for (const [name, makeTarget] of CODEGEN) {
    expect(typeof compile({ forward: fwd }, inputs, { target: makeTarget() }), `${name} codegen`).toBe('function');
  }
}
...
describe('large models end-to-end (compiled == eager, all backends)', () => {
  it('Transformer encoder stack (attention + layernorm + gelu FFN + residual)', async () => {
    ...
    await expectMatchesEager(fwd, [mkInput(rng, [B, SEQ, D])]);
  });
  ...
  it('CNN classifier (conv + batchnorm + relu + maxpool + linear)', async () => {
    ...
    await expectMatchesEager(fwd, [mkInput(rng, [1, 3, 8, 8])]);
  });
  ...
  it('BatchNorm in training mode (batch statistics) compiles == eager', async () => {
    ...
    await expectMatchesEager((x) => bn2d.forward(x), [mkInput(rng, [4, 8, 6, 6])]);
    const bn1d = new nn.BatchNorm1d(16);
    await expectMatchesEager((x) => bn1d.forward(x), [mkInput(rng, [10, 16])]);
  });
  ...
  it('deep MLP (6 linear layers + gelu)', async () => {
    ...
    await expectMatchesEager(fwd, [mkInput(rng, [4, 32])]);
  });
