...
describe.skipIf(!cudaDeps)('CUDA performance: scheduling speeds up matmul on real GPU', () => {
  it('scheduled matmul is much faster than unscheduled baseline (1-thread)', async () => {
    ...
    expect(base / sched, `scheduling speedup collapsed: ${(base / sched).toFixed(1)}x`).toBeGreaterThan(10);
    expect(base / autotune, `autotune speedup collapsed: ${(base / autotune).toFixed(1)}x`).toBeGreaterThan(10);
    expect(sched / autotune, `autotune is slower than the default schedule: ${(sched / autotune).toFixed(2)}x`).toBeGreaterThan(0.9);
  }, 60000);
});
